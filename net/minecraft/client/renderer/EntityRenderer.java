...
implements IResourceManagerReloadListener {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (!this.mc.gameSettings.ofProfiler) return;
    ...
    private void renderCloudsCheck(RenderGlobal renderGlobalIn, float partialTicks, int pass) {
        ...
        if (!Shaders.shouldRenderClouds(this.mc.gameSettings)) return;
    ...
    private void frameInit() {
        ...
        if (this.mc.currentScreen.getClass() != GuiChat.class) return;
        this.mc.displayGuiScreen(new GuiChatOF((GuiChat)this.mc.currentScreen));
    }

    private void frameFinish() {
        if (this.mc.theWorld == null) return;
        if (!Config.isShowGlErrors()) return;
        if (!TimedEvent.isActive("CheckGlErrorFrameFinish", 10000L)) return;
        int i = GlStateManager.glGetError();
        if (i == 0) return;
        if (!GlErrors.isEnabled(i)) return;
    ...
    private void checkLoadVisibleChunks(Entity p_checkLoadVisibleChunks_1_, float p_checkLoadVisibleChunks_2_, ICamera p_checkLoadVisibleChunks_3_, boolean p_checkLoadVisibleChunks_4_) {
        ...
        if (!Keyboard.isKeyDown((int)61)) return;
        if (!Keyboard.isKeyDown((int)38)) return;
        if (this.mc.currentScreen != null) {
            return;
        }
    ...
    private void loadAllVisibleChunks(Entity p_loadAllVisibleChunks_1_, double p_loadAllVisibleChunks_2_, ICamera p_loadAllVisibleChunks_4_, boolean p_loadAllVisibleChunks_5_) {
        ...
        try {
            ...
            do {
                flag1 = false;
                for (int j1 = 0; j1 < 100; i1 -= renderglobal.getCountChunksToUpdate(), ++j1) {
